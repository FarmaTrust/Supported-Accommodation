...
export default function StaffWorkspacePage() {
  ...
  return <div className="mx-auto max-w-[1120px] space-y-6 pb-20"><PageHeader eyebrow="Staff self-service" title="My staff workspace" description="Manage your personal workforce requests and view supervision information. Requests are reviewed independently by an authorised manager." action={<StaffRequestDialog entityId={entityId} />} />
    <section className="surface grid gap-5 p-6 sm:grid-cols-[auto_1fr_auto] sm:items-center"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-accent text-accent-foreground"><UserRound className="h-6 w-6" /></div><div><p className="text-xl font-extrabold tracking-[-0.04em]">{data.profile.fullName}</p><p className="mt-1 text-sm font-semibold text-muted-foreground">{data.profile.jobTitle}</p><p className="mt-3 text-xs text-muted-foreground">{data.profile.employeeNumber ? `Employee no. ${data.profile.employeeNumber}` : "Employee number not recorded"} · {readable(data.profile.employmentType)}</p></div><StatusBadge status={data.profile.status} /></section>
    <div className="grid gap-6 lg:grid-cols-2"><section className="surface p-6"><div className="mb-5 flex items-center justify-between gap-4"><div><p className="eyebrow">Requests</p><h2 className="text-xl font-extrabold tracking-[-0.04em]">My submitted requests</h2></div><ClipboardList className="h-5 w-5 text-primary" /></div>{data.requests.length ? <div className="space-y-3">{data.requests.map(request => <article key={request.id} className="rounded-2xl border border-border/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{readable(request.requestType)}</p><p className="mt-1 text-xs text-muted-foreground">Submitted {asDate(request.createdAt)}{request.startsAt ? ` · ${asDate(request.startsAt)}${request.endsAt ? ` to ${asDate(request.endsAt)}` : ""}` : ""}</p></div><StatusBadge status={request.status} /></div></article>)}</div> : <div className="rounded-2xl bg-muted/50 p-5 text-sm text-muted-foreground">No requests are recorded. Use <strong className="font-semibold text-foreground">New request</strong> to submit a personal change, absence or availability request.</div>}</section>
      <section className="surface p-6"><div className="mb-5 flex items-center justify-between gap-4"><div><p className="eyebrow">Supervision</p><h2 className="text-xl font-extrabold tracking-[-0.04em]">My sessions</h2></div><CalendarDays className="h-5 w-5 text-primary" /></div>{data.supervisions.length ? <div className="space-y-3">{data.supervisions.map(session => <article key={session.id} className="rounded-2xl border border-border/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">Scheduled {asDate(session.scheduledAt)}</p><p className="mt-1 text-xs text-muted-foreground">{session.location || "Location to be confirmed"}</p></div><StatusBadge status={session.status} /></div>{session.sharedNotes ? <p className="mt-3 line-clamp-3 text-sm text-muted-foreground">{session.sharedNotes}</p> : null}</article>)}</div> : <div className="rounded-2xl bg-muted/50 p-5 text-sm text-muted-foreground">No supervision sessions are currently shared with you.</div>}</section></div>
